...
def download_blazeface_models():
    ...
    """
    os.makedirs(
        os.path.dirname(BLAZEFACE_MODEL), exist_ok=True
    ...
    if not os.path.exists(BLAZEFACE_MODEL):
        ...
        try:
            ...
            print(
                "Manual download: Save to ./models/res10_300x300_ssd_iter_140000.caffemodel"
            )
            sys.exit(1)
    else:
...
def load_blazeface():
    ...
    try:
        ...
        print(
            f"BlazeFace loaded! Detection confidence threshold: {DETECTION_CONFIDENCE}"
        )
        return net
    except Exception as e:
...
def load_mobilefacenet():
    ...
    try:
        ...
        print(
            "Check PTH download; ensure torch installed (pip install torch torchvision)."
        )
        sys.exit(1)
...
def capture_face(blazeface_net, mobilefacenet_model, mode="add"):
    ...
    What?
      1. Opens webcam (cv2.VideoCapture).
    ...
    How?
      ...
      - Display: 640x480 downsampled (speeds up rendering; detection on full res).
      - FPS display: deque for 30-frame rolling avg; text overlay with outline for visibility.
    Speed: ~30 FPS (5ms detect + 5ms render + 20ms idle).
    Returns: Normalized embedding (np.array 128-dim) or None (ESC/error).
    Trade-off: Captures first face only; multi-face needs UI selection (future feature).
    """
    ...
    while True:
        ...
        cv2.putText(
            display_frame, fps_text, (12, 32), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3
        )  # Black outline
        cv2.putText(
            display_frame,
            fps_text,
...
def add_face(blazeface_net, mobilefacenet_model, db):
    ...
    if name in db:
        overwrite = (
            input(f"'{name}' already exists. Overwrite? (y/n): ").strip().lower()
        )
        if overwrite != "y":
            print("Cancelled.")
            return
